...
processpool
{
	...
	public: /*单体模式，以保证程序最多创建一个processpool实例，这是程序正确处理信号的必要条件*/ 
	...
	while(!m_stop)
	{
		...
		if((sockfd==pipefd)＆＆(events[i].events＆EPOLLIN))
		{
			...
			{ 
				...
				/*模板类T必须实现init方法，以初始化一个客户连接。我们直接使用connfd来索引逻辑处理对象（T类型的对象），以提高程序效率*/ 					users[connfd].init(m_epollfd,connfd,client_address);
			}
		}
		 /*下面处理子进程接收到的信号*/
		else  if((sockfd==sig_pipefd[0])＆＆(events[i].events＆EPOLLIN))
		{
			...
			else
			{
				for(int  i=0;i＜ret;++i)
				{
					switch(signals[i])
					{
						case  SIGCHLD:{ pid_t  pid; int  stat; while((pid=waitpid(-1,＆stat,WNOHANG))＞0){ continue;} break;}
						case  SIGTERM: case  SIGINT:{ m_stop=true; break;}
						default:{ break;}
					}
				}
			}
					...
					for(int  i=0;i＜ret;++i)
					{
						switch(signals[i])
						{
							...
							{ 
								...
								当然，通知子进程结束更好的方法是向父、子进程之间的通信管道发送特殊数据，读者不妨自己实现之*/
